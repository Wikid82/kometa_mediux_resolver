...
def fetch_set_assets(
    api_base: str, set_id: str, api_key: str | None = None, timeout=10
) -> list[dict[str, Any]]:
...
def fetch_set_assets_with_scrape(
    ...
    for item in extracted:
        if isinstance(item, dict):
            normalized.append(
                {"id": item.get("id"), "name": None, "type": item.get("fileType"), "raw": item}
            )
        else:
...
def probe_url(url: str, api_key: str | None = None, timeout: int = 10) -> dict[str, Any]:
    """GET the URL and return status and truncated body for debugging."""
    # Simple probe with no caching (caching is handled by caller via sqlite helper)
    headers = {}
    if api_key:
        headers["Authorization"] = f"Bearer {api_key}"
    try:
...
def get_cached_probe(conn: sqlite3.Connection, url: str, max_age: int) -> dict[str, Any] | None:
    ...
    try:
        ...
        if not row:
            return None
        status, body, last_checked = row
        if int(time.time()) - int(last_checked) > max_age:
            return None
        return {
            "url": url,
            "status": status,
...
def set_cached_probe(
    ...
    try:
        cur = conn.cursor()
        cur.execute(
            "REPLACE INTO probe_cache(url, status, body, last_checked) VALUES (?,?,?,?)",
...
def get_recently_aired_from_sonarr(sonarr_url: str, api_key: str, days: int = 7) -> list[int]:
    """Query Sonarr calendar and return a list of TVDB ids of recently aired shows.

    This calls the Sonarr Calendar API: /api/calendar?start=YYYY-MM-DD&end=YYYY-MM-DD
    and extracts the `series.tvdbId` where available.
    """
    if not sonarr_url:
        return []
    headers = {}
    ...
    url = f"{sonarr_url.rstrip('/')}/api/calendar?start={start.isoformat()}&end={end.isoformat()}"
    try:
        r = requests.get(url, headers=headers, timeout=10)
        if r.status_code != 200:
            logging.warning("Sonarr calendar returned %s", r.status_code)
            return []
        data = r.json()
        tvdb_ids = set()
        for ep in data:
            # ep may contain a nested 'series' object with 'tvdbId'
            series = ep.get("series") or {}
            tvdb = series.get("tvdbId") or series.get("tvdbId", None)
            if tvdb:
                try:
                    tvdb_ids.add(int(tvdb))
                except Exception:
                    pass
        return list(tvdb_ids)
    except Exception as e:
        logging.warning("Failed to query Sonarr: %s", e)
        return []


# Activity tracking for live heartbeat
_last_activity = time.time()
_processed_count = 0
_activity_lock = threading.Lock()


def touch_activity(count_inc: int = 0) -> None:
    """Update activity timestamp and optionally increment processed count."""
    global _last_activity, _processed_count
    with _activity_lock:
        _last_activity = time.time()
        if count_inc:
            _processed_count += int(count_inc)
...
def propose_changes_for_file(
    ...
    for sid in set_ids:
        assets_cache[sid] = fetch_set_assets_with_scrape(
            api_base, sid, api_key, use_scrape=use_scrape, mediux_opts=mediux_opts
        )
        ...
        for path, node in nodes:
            ...
            for sid, assets in assets_cache.items():
                ...
                for aid in candidates:
                    ...
                    probe = None
                    if cache_conn:
                        cached = get_cached_probe(cache_conn, candidate_url, cache_ttl)
                        if cached:
                            probe = cached
                    if not probe:
...
def scan_root(
    ...
    all_files = list(root.rglob("*.yml")) + list(root.rglob("*.yaml"))

    # If Sonarr provided recently-aired tvdb ids, prioritize files that contain those ids
    prioritized = []
    normal = []
    for p in all_files:
        if not match_file(p):
            continue
        # quick path: if we have sonarr ids, read the file text and check for any tvdb id
        is_prior = False
        try:
            if sonarr_ids:
    ...
    # scan prioritized files first, then the rest
    for p in prioritized + normal:
        try:
            # mark activity when starting to process a file
            try:
                touch_activity(0)
            except Exception:
                pass
            r = propose_changes_for_file(
                p,
                api_base,
                api_key,
    ...
    for entry in changes:
        ...
        for c in entry["changes"]:
            ...
            if require_probe_ok and probe:
                ...
                if status != 200:
                    logging.warning(
                        "Skipping change for %s path %s because probe status is %s",
                        file_path,
                        path,
                        status,
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        if bak and bak.exists():
                            ...
                            logging.info(
                                "Restored backup %s to %s after failed write", bak, file_path
                            )
                    except Exception:
...
def main(argv=None):
    ...
    parser.add_argument(
        "--root", "-r", default=".", help="Root path to scan (default: repository root)"
    )
    ...
    parser.add_argument(
        "--output", default=".kometa_mediux_changes.json", help="Summary output (JSON)"
    )
    ...
    parser.add_argument(
        ...
        dest="exclude_config",
        action="store_false",
        help="Do not exclude files named config*",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1048576,
        help="Max bytes per log file before rotation (default: 1048576)",
    )
    parser.add_argument(
        "--log-backups",
        dest="log_backups",
    ...
    parser.add_argument(
        "--probe-set",
        default=None,
        help="If provided, probe MediUX endpoints for this set id and exit",
    )
    parser.add_argument(
        "--file",
        dest="file_filter",
        default=None,
        help="If provided, only scan files matching this name or path suffix",
    )
    parser.add_argument(
        "--probe-asset",
        dest="probe_asset",
        default=None,
        help="If provided, probe this asset URL and exit",
    )
    parser.add_argument(
        "--require-probe-ok",
        dest="require_probe_ok",
        action="store_true",
        help="Only apply additions if asset probe returns HTTP 200",
    )
    parser.add_argument(
        "--use-scrape",
        dest="use_scrape",
        action="store_true",
        help="When API listing fails, use Selenium scraper fallback to fetch MediUX YAML",
    )
    parser.add_argument(
        "--mediux-username",
        dest="mediux_username",
        default=None,
        help="Mediux username (overrides config file)",
    )
    parser.add_argument(
        "--mediux-password",
        dest="mediux_password",
        default=None,
        help="Mediux password (overrides config file)",
    )
    parser.add_argument(
        "--mediux-nickname",
        dest="mediux_nickname",
        default=None,
        help="Mediux nickname (overrides config file)",
    )
    parser.add_argument(
        "--profile-path", dest="profile_path", default=None, help="Chrome profile path to reuse"
    )
    parser.add_argument(
        "--chromedriver-path",
    ...
    parser.add_argument(
        ...
        dest="apply_backup",
        action="store_true",
        help="When applying, keep a timestamped backup of the original file (overrides config)",
    )
    parser.add_argument(
        "--sonarr-url",
        dest="sonarr_url",
        default=None,
        help="Optional Sonarr URL (e.g. http://sonarr.local:8989)",
    )
    parser.add_argument(
        "--sonarr-api-key",
        dest="sonarr_api_key",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Lookback window (days) for Sonarr calendar (overrides config)",
    )
    parser.add_argument(
        "--cache-db",
        dest="cache_db",
        default=None,
        help="Path to sqlite cache DB (relative to resolver folder or absolute)",
    )
    parser.add_argument(
        "--cache-ttl",
        dest="cache_ttl",
        type=int,
        default=None,
        help="Cache TTL in seconds for probe results (overrides config)",
    )
    ...
    args.log_max_bytes = getattr(args, "log_max_bytes", None) or int(
        cfg.get("log_max_bytes", 1048576)
    )
    args.log_backups = getattr(args, "log_backups", None) or int(cfg.get("log_backups", 5))

    # Sonarr and cache config
    args.sonarr_url = args.sonarr_url or cfg.get("sonarr_url")
    args.sonarr_api_key = args.sonarr_api_key or cfg.get("sonarr_api_key")
    args.sonarr_days = args.sonarr_days or int(cfg.get("sonarr_days", 7))
    args.cache_db = args.cache_db or cfg.get("cache_db", "config/cache.db")
    args.cache_ttl = args.cache_ttl or int(
        cfg.get("cache_ttl_seconds", cfg.get("cache_ttl_seconds", 86400))
    )
    ...
    if args.log_file:
        ...
        if RotatingFileHandler:
            ...
            logging.basicConfig(
                level=level, format="%(asctime)s %(levelname)s: %(message)s", filename=args.log_file
            )
    else:
    ...
    if args.probe_asset:
        ...
        out.write_text(
            json.dumps({"probe_asset": args.probe_asset, "probe": probe}, indent=2),
            encoding="utf-8",
        )
    ...
    if args.probe_set:
        ...
        if args.use_scrape:
            assets = fetch_set_assets_with_scrape(
                args.api_base, args.probe_set, api_key, use_scrape=True, mediux_opts=mediux_opts
            )
        else:
        ...
        out = Path(args.output)
        out.write_text(
            json.dumps({"probe_set": args.probe_set, "assets": assets}, indent=2), encoding="utf-8"
        )
        logging.info("Wrote probe output to %s", out)
        return 0
    ...
    # initialize cache DB if configured
    cache_conn = None
    try:
        if args.cache_db:
            cache_conn = init_cache(args.cache_db)
    except Exception as e:
        logging.warning("Failed to initialize cache DB %s: %s", args.cache_db, e)
    ...
    try:
        if args.sonarr_url:
            ...
            logging.info("Sonarr returned %d recently-aired tvdb ids", len(sonarr_ids))
    except Exception as e:
        logging.warning("Sonarr lookup failed: %s", e)

    # start a heartbeat thread to log periodic progress so users can see activity
    heartbeat_interval = (
        int(cfg.get("heartbeat_interval_seconds", 15)) if isinstance(cfg, dict) else 15
    )
    ...
    def _heartbeat():
        ...
        while not stop_event.wait(heartbeat_interval):
            ...
            ago = time.time() - last
            logging.info("Heartbeat: processed %d files; last activity %.1fs ago", proc, ago)

    hb_thread = threading.Thread(target=_heartbeat, name="kometa-heartbeat", daemon=True)
    hb_thread.start()
    ...
    results = scan_root(
        ...
        mediux_opts=mediux_opts,
        cache_conn=cache_conn,
        cache_ttl=getattr(args, "cache_ttl", 86400),
    ...
    if args.apply:
        ...
        apply_changes(
            ...
            create_backup=bool(getattr(args, "apply_backup", False)),
        )
    else:
