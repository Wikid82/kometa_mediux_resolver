...
class TestWorkingHighImpactCoverage:
    ...
    def test_touch_activity_correct_usage(self):
        ...
        # touch_activity expects a count increment, not a string key
        kmr.touch_activity("processed_files", 1)
    ...
    def test_get_recently_aired_from_sonarr_correct_params(self):
        ...
        with patch("kometa_mediux_resolver.requests.get") as mock_get:
            ...
            result = kmr.get_recently_aired_from_sonarr(config, api_key)
            assert result == []
    ...
    def test_gather_yaml_metadata_paths_working(self):
        """Test gather_yaml_metadata_paths with real directory."""
        # Create a temporary directory with yaml files
        with tempfile.TemporaryDirectory() as temp_dir:
            # Create some yaml files
            (Path(temp_dir) / "test1.yml").write_text("metadata: test")
            (Path(temp_dir) / "test2.yaml").write_text("metadata: test")
            (Path(temp_dir) / "test.txt").write_text("not yaml")

            result = kmr.gather_yaml_metadata_paths(temp_dir)
            assert len(result) == 2  # Only .yml and .yaml files
            assert all(str(p).endswith((".yml", ".yaml")) for p in result)

    def test_probe_url_working(self):
        """Test probe_url function."""
        with patch("kometa_mediux_resolver.requests.head") as mock_head:
            mock_response = Mock()
