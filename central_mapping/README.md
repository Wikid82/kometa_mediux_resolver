...
2. `central_mapping/write_path_map.yml` entries (set -> tvdb -> filename)
3. Heuristics implemented in the script

Notes
- The mapping folder is intentionally kept in the repo by default so the script can use these mappings as a fallback during automated runs. If you prefer this to be local-only (learning/dev mode), add `central_mapping/` to `.gitignore` and `.dockerignore`.
